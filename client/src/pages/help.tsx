...
export default function Help() {
  ...
  return (
    <div className="space-y-6 pb-24 md:pb-6">
      ...
      {/* Filters the FAQ list below. Previously this input had no value and no
          onChange — typing in it did nothing at all. */}
        ...
        <CardContent>
          ...
          {visibleFaqs.length === 0 && (
            <p className="text-sm text-muted-foreground py-6 text-center">
              No articles match "{query}". Try a different word, or email support below.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      {/*
        This was three cards — "View Docs", "Watch Videos" and "Start Chat" —
        each a button with no onClick, two of them carrying an ExternalLink icon
        that promised navigation. There is no documentation site, no tutorials
        library and no live chat, so wiring them was not possible; offering them
        was the bug. Removed rather than faked. Email support is real, so that
        is what the page now offers, in the block below.
      */}
      <Card className="bg-gradient-to-r from-primary/10 to-chart-2/10 border-0">
        <CardContent className="p-6 flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <p className="text-muted-foreground mt-1">
              Can't find what you're looking for? Email us and we'll come back to you.
            </p>
          </div>
          <Button className="rounded-full gap-2" asChild data-testid="button-contact-support">
            <a href={`mailto:${COMPANY.contactEmail}?subject=${encodeURIComponent("Materialized support request")}`}>
